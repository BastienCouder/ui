  ...
  it('renders different variants and sizes together', () => {
    cy.mount(
      <>
        ...
        <Badge variant="accent" size="lg">Accent Large</Badge>
      </>
    );
    cy.get('span').eq(0).should('have.class', 'bg-warning').and('have.class', 'h-5');
    cy.get('span').eq(1).should('have.class', 'bg-neutral').and('have.class', 'h-6');
    cy.get('span').eq(2).should('have.class', 'bg-accent').and('have.class', 'h-7');
  });
});

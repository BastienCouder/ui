      ...
      >
        ...
        {localValues.map((numberStep, index) => (
          <SliderPrimitive.Thumb
            key={index}
            className={cn(
              "border-primary/50 bg-bg focus-visible:ring-ring block w-4 h-4 rounded-full border shadow transition-colors focus-visible:outline-none focus-visible:ring-1",
            )}
          >
            {hoveredThumbIndex && formatLabel && (
              <div
                className={cn(
                  ...
                  "bg-popover text-popover-fg absolute z-30 w-max items-center justify-items-center rounded-md border px-2 py-1 text-center shadow-sm",
                )}
              >
